(function () {
    ...
    const FIXED_UNTIL = "Model";
    const ROW_HEIGHT = 38;
    const BUFFER_ROWS = 10;
    const ALL_COLUMNS = ["상태", "Lot", "CODE", "WO", "S/N", "Customer", "Line", "Model", "FSC", "EFEM", "TM", "PM", "SU", "Harness", "Stage", "Tuning", "생산시작일", "Tuning시작일", "생산완료일", "Remark"];
    const FIXED_COLUMNS = [];
    for (const c of ALL_COLUMNS) {
        FIXED_COLUMNS.push(c);
        if (c === FIXED_UNTIL) break;
    }
    const NON_HIDEABLE_COLUMNS = new Set(FIXED_COLUMNS);
    ...
    function uniqueSorted(values) {
        return Array.from(new Set(values.filter((v) => String(v || "").trim() !== ""))).sort((a, b) => String(a).localeCompare(String(b), "ko"));
    }
    ...
    function getVisibleColumns() {
        return ALL_COLUMNS.filter((col) => visibleColumns.has(col));
    }
    ...
    function getColumnWidthRange(col) {
        let minWidth = 100;
        let maxWidth = 260;
        if (col === "상태") { minWidth = 120; maxWidth = 120; }
        else if (col === "Remark") { minWidth = 360; maxWidth = 900; }
        else if (col === "WO") { minWidth = 140; maxWidth = 200; }
        else if (["Lot", "CODE"].includes(col)) { minWidth = 110; maxWidth = 150; }
        else if (["S/N", "FSC"].includes(col)) { minWidth = 130; maxWidth = 220; }
        else if (["EFEM", "TM", "PM", "SU", "Harness", "Stage", "Tuning"].includes(col)) { minWidth = 110; maxWidth = 180; }
        else if (["Customer", "Model"].includes(col)) { minWidth = 150; maxWidth = 340; }
        else if (["Line"].includes(col)) { minWidth = 140; maxWidth = 240; }
        else if (["생산시작일", "Tuning시작일", "생산완료일"].includes(col)) { minWidth = 140; maxWidth = 140; }
        return { minWidth, maxWidth };
    }

    function computeColumnWidths() {
        const widths = {};
        const sourceRows = filteredData.length ? filteredData : rawData;
    ...
    function renderTableStructure() {
        ...
        let totalWidth = 0;
        getVisibleColumns().forEach((col) => { totalWidth += getColumnWidth(col); });
    ...
    function sortData(data) {
        ...
        return data.slice().sort(function (a, b) {
            ...
            if (!Number.isNaN(ad) && !Number.isNaN(bd) && String(av).length >= 8 && String(bv).length >= 8) {
                ...
                const an = Number(av);
                const bn = Number(bv);
                if (!Number.isNaN(an) && !Number.isNaN(bn) && String(av).trim() !== "" && String(bv).trim() !== "") result = an - bn;
    ...
    function applyFilters(options) {
        ...
        let data = rawData.filter(function (row) {
            const fullText = normalizeText([
                row["상태"], row["Lot"], row["CODE"], row["WO"], row["S/N"], row["Customer"], row["Line"], row["Model"], row["FSC"],
                row["EFEM"], row["TM"], row["PM"], row["SU"], row["Harness"], row["Stage"], row["Tuning"], row["Remark"]
            ].join(" "));
    ...
    function isBetween(target, start, end) { return !!(target && start && end && start <= target && target <= end); }
    ...
    document.getElementById("filterStatus").addEventListener("change", function () { applyFilters({ resetScrollTop: true }); });
    document.getElementById("filterCustomer").addEventListener("change", function () { applyFilters({ resetScrollTop: true }); });
    document.getElementById("filterLine").addEventListener("change", function () { applyFilters({ resetScrollTop: true }); });
